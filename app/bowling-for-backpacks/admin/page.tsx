...
const teamLink = (registration: BowlingRegistrationRecord) =>
  `${bowlingEventConfig.teamBaseUrl}/${registration.id}`;

const buyerName = (registration: BowlingRegistrationRecord) =>
  ...
  return (
    <section className="rounded-sm border border-bfb-ink/10 bg-white p-6 shadow-soft">
      <div className="flex items-start gap-3">
        <AlertCircle aria-hidden="true" className="mt-1 text-bfb-blue" size={22} />
        <div>
          <p className="bfb-eyebrow">Follow-up queue</p>
          <h2 className="mt-2 font-heading text-2xl font-black text-bfb-ink">Registrations that need a human touch</h2>
          <p className="mt-2 text-sm leading-6 text-bfb-ink/65">Use the action buttons to email supporters or open the team link without digging.</p>
        </div>
      </div>
      <div className="mt-6 divide-y divide-bfb-ink/10">
        {followUps.length === 0 ? <p className="rounded-sm bg-bfb-green/15 p-4 text-sm font-bold text-bfb-ink">No follow-ups right now. Beautiful little admin miracle.</p> : followUps.map((registration) => (
          <div key={registration.id} className="py-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="font-heading text-lg font-black text-bfb-ink">{registration.organization || buyerName(registration)}</p>
                <p className="mt-1 text-sm leading-6 text-bfb-ink/65">{registration.packageName} · {registration.buyerEmail}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {needsPaymentFollowUp(registration) ? <StatusPill tone="warning">Payment</StatusPill> : null}
                {needsBowlerFollowUp(registration) ? <StatusPill tone="warning">Bowlers</StatusPill> : null}
                {needsExportFollowUp(registration) ? <StatusPill>Export</StatusPill> : null}
              </div>
            </div>
            <p className="mt-3 text-sm leading-6 text-bfb-ink/65">Confirmation: <strong>{registration.id}</strong> · Amount: <strong>{formatMoney(registration.grandTotal)}</strong>{registration.sessionName ? <> · Session: <strong>{registration.sessionName}</strong></> : null}</p>
  ...
  return (
    <section className="rounded-sm border border-bfb-ink/10 bg-white p-6 shadow-soft">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="bfb-eyebrow">All registrations</p>
          <h2 className="mt-2 font-heading text-2xl font-black text-bfb-ink">Registration view</h2>
          <p className="mt-2 text-sm leading-6 text-bfb-ink/65">Read-only for now. Action buttons open email or team links without changing database records.</p>
        </div>
        <Link href="/api/bowling/export" className="bfb-secondary inline-flex items-center gap-2"><Download aria-hidden="true" size={16} />Export CSV</Link>
      </div>
      <div className="mt-6 overflow-x-auto">
        <table className="min-w-full border-collapse text-left text-sm">
          <thead><tr className="border-b border-bfb-ink/10 text-xs uppercase tracking-wide text-bfb-ink/50"><th className="py-3 pr-4 font-bold">Name</th><th className="py-3 pr-4 font-bold">Type</th><th className="py-3 pr-4 font-bold">Amount</th><th className="py-3 pr-4 font-bold">Payment</th><th className="py-3 pr-4 font-bold">Bowlers</th><th className="py-3 pr-4 font-bold">Created</th><th className="py-3 pr-4 font-bold">Actions</th></tr></thead>
          <tbody className="divide-y divide-bfb-ink/10">
            {registrations.map((registration) => (
              <tr key={registration.id}>
                <td className="py-4 pr-4 align-top"><p className="font-bold text-bfb-ink">{registration.organization || buyerName(registration)}</p><p className="mt-1 text-xs text-bfb-ink/60">{registration.buyerEmail}</p><p className="mt-1 text-xs text-bfb-ink/50">{registration.id}</p></td>
                <td className="py-4 pr-4 align-top text-bfb-ink/75">{registration.packageName}</td>
                <td className="py-4 pr-4 align-top font-bold text-bfb-ink">{formatMoney(registration.grandTotal)}</td>
                <td className="py-4 pr-4 align-top"><StatusPill tone={registration.paymentStatus === "paid" ? "good" : "warning"}>{registration.paymentStatus.replace(/_/g, " ")}</StatusPill></td>
                <td className="py-4 pr-4 align-top text-bfb-ink/75">{registration.laneCount > 0 ? `${completedBowlers(registration)} / ${bowlingEventConfig.teamSize}` : "—"}</td>
                <td className="py-4 pr-4 align-top text-bfb-ink/65">{formatDate(registration.createdAt)}</td>
                <td className="py-4 pr-4 align-top"><ActionButtons registration={registration} /></td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <main className="min-h-screen bg-bfb-cream py-10">
      <div className="bfb-shell">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="bfb-eyebrow">City Center event admin</p><h1 className="mt-3 font-heading text-4xl font-black leading-tight text-bfb-ink sm:text-5xl">Bowling for Backpacks dashboard</h1><p className="mt-4 max-w-3xl text-base leading-7 text-bfb-ink/70">A simple command center for registrations, payments, bowler follow-up, and export readiness.</p></div><Link href="/bowling-for-backpacks" className="bfb-secondary">View event page</Link></div>
        {!configured ? <div className="mt-8 rounded-sm border border-yellow-300 bg-yellow-50 p-5 text-yellow-950"><p className="font-heading text-lg font-black">Database not connected yet</p><p className="mt-2 text-sm leading-6">Supabase environment variables are not configured, so this dashboard will be empty until production data is connected.</p></div> : null}
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><StatCard label="Registrations" value={String(registrations.length)} helper="Total records received." /><StatCard label="Projected revenue" value={formatMoney(totalRevenue)} helper="Includes teams, sponsors, lanes, and gifts." /><StatCard label="Lanes reserved" value={String(totalLanes)} helper="Across both bowling sessions." /><StatCard label="Needs follow-up" value={String(paymentFollowUps + missingBowlers)} helper="Payment or bowler details to resolve." /></div>
        <div className="mt-8 grid gap-4 md:grid-cols-3"><div className="rounded-sm bg-bfb-navy p-5 text-white"><CreditCard aria-hidden="true" size={24} /><p className="mt-4 font-heading text-xl font-black">Payment follow-up</p><p className="mt-2 text-sm leading-6 text-white/75">{paymentFollowUps} registration(s) need payment confirmation, invoice, or check follow-up.</p></div><div className="rounded-sm bg-bfb-blue p-5 text-white"><UsersRound aria-hidden="true" size={24} /><p className="mt-4 font-heading text-xl font-black">Bowler follow-up</p><p className="mt-2 text-sm leading-6 text-white/80">{missingBowlers} team(s) still need bowler names completed.</p></div><div className="rounded-sm bg-white p-5 text-bfb-ink shadow-sm"><Mail aria-hidden="true" className="text-bfb-green" size={24} /><p className="mt-4 font-heading text-xl font-black">Contact</p><p className="mt-2 text-sm leading-6 text-bfb-ink/65">Primary event contact: {bowlingEventConfig.contactName}</p></div></div>
        <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)]"><FollowUpList registrations={registrations} /><RegistrationsTable registrations={registrations} /></div>
      </div>
    </main>
  );
}
